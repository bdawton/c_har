...
for nh_a in nhoa_arr:
    for gh_a in ghoa_arr:
        for eh_a in ehoa_arr:
            ...
            sequence_root_1 = PRS.PRS(mf_prs_params['p_1'], mf_prs_params['d_1'], mf_prs_params['k_1'], t, f_prs, 0)
            sequence_1 = np.tile(sequence_root_1, int(f_sim / f_prs))
            sequence_root_2 = PRS.PRS(mf_prs_params['p_2'], mf_prs_params['d_2'], mf_prs_params['k_2'], t, f_prs, 0)
            sequence_2 = np.tile(sequence_root_2, int(f_sim / f_prs))
            seq_num = mf_prs_params['seq_num']
            if seq_num == 1:
...
df_output_log.to_excel("gyro_final_approach_3class_all.xlsx")
