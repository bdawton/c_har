...
def bit_chopper(i,r):
        ...
        abs_i = np.abs(i)
        bi = np.fromstring(np.binary_repr(abs_i), dtype='S1').astype(int)
        cut = len(bi)-r
        if cut>=0:

            bi = bi[:-cut]
            bc = bi.dot(2**np.arange(bi.size)[::-1])
        else:
            bc = 0
...
def bytes_bits_needed(m):


        if m == 0: #Check this is still valid
            return 1,1
        elif m < 0:
            m = np.abs(m)-1
            if m <= 0:
        ...
        n_bits = int(log(m, 2)) + 2 # +2 because signed integer
        n_bytes = int(log(m, 256)) + 1
        return n_bytes, n_bits


def int_for_classifier(c):


#ADD SOME LOGIC TO KEEP THE SMALL NUMBERS SMALL AS THEY ARE
#AND DEAL WITH THE MATH LOGIC ERRROR IN BYTES_BITS_NEEDED
#THEN CHECK PERFORMANCE !

        nb = bytes_bits_needed(c)[0]
        if nb == 1:
            c = np.int8(c)
        elif nb <=2:
            c = np.int16(c)
...
def to_int(thre):
    ...
    #thr_range = np.ptp(thr_list)
    #b_range = bytes_bits_needed(thr_range)[1]
   # b_range = b_range*8
    ...
    for thr in thre:
        ...
        #thr[1] = int_for_classifier(int(thr[1]*thr_range))
        thr[1] = int_for_classifier(bit_chopper(int(thr[1]),b_range))

    return thre


def flatten_tree(tree, int_toggle):
    flat = []
...
def flatten_forest(trees, int_toggle):
    tree_roots = []
    tree_offset = 0
    forest_nodes = []

    for tree in trees:
        flat = flatten_tree(tree, int_toggle)
...
def generate_c_forest(forest, name='myclassifier', dtype='float'):
    ...
    #return '\n\n'.join([head, tree_roots, forest_struct, inline])



class Wrapper:
    def __init__(self, estimator, classifier, int_toggle, dtype='float'):
        ...
        self.forest_ = flatten_forest(trees, int_toggle)
        self.forest_ = remove_duplicate_leaves(self.forest_)

        if classifier == 'pymodule':
            # FIXME: use Nodes,Roots directly, as np Array
